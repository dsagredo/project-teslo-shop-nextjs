...
export const paypalCheckPayment = async (
    ...
    const { invoice_number: orderId } = purchase_units[0];
...
const getPayPalToken = async (): Promise<string | null> => {
    ...
    const AUTH_URL = process.env.PAYPAL_OAUTH_URL ?? '';
...
const verifyPayPalPayment = async (
    ...
    const PAYPAL_ORDER = `${process.env.PAYPAL_ORDERS_URL}/${paypalTransactionId}`;
    ...
    try {
        const resp = await fetch(PAYPAL_ORDER, {
            ...requestOptions,
            cache: 'no-store',
        }).then((response: Response): Promise<any> => response.json());
        return resp;
